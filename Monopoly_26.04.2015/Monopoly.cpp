...
static WNDPROC OldGroupBoxProc;					// îáðàáîò÷èê 
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		...
		hWndGroupBox = CreateWindowEx(0, _T("BUTTON"), _T("Ýòî GroupBox"),
			WS_CHILD | WS_VISIBLE | BS_GROUPBOX,
			10, 10, 400, 300,
			hWnd, 0, hInst, 0);

		hWndOkBtn = CreateWindowEx(0, _T("BUTTON"), _T("Ýòî êíîïêà â GroupBoxå"),
			WS_CHILD | WS_VISIBLE,
			10, 20, 250, 25,
	...
	case WM_COMMAND:
		...
		//if (wmEvent == BN_CLICKED && wmId >= ID_CHECK_START && wmId <= ID_CHECK_END)
		//	{ 
		...
		//1//EnableGroupBox(currPlayerName());
		//
		//EnableRollDices();
